...
The clean-room procedure referenced by this policy is defined in
[`CLEAN_ROOM.md`](CLEAN_ROOM.md).

After the explicit Kaname enforcement boundary described below, every change
merged into Hikoboshi must have complete, reviewable provenance. This
requirement applies to code, tests, documentation, dependencies, build
...
claim that Kaname observed work it did not observe. This is a forward-only
temporary exception, not reconstructed provenance and not a waiver for any
external patch.
...
later conversion. The public record and local validator are necessary
projections of a completed history; neither creates missing history or makes a
retrospectively reconstructed change admissible.
...
record is admissible only after a maintainer orders a clean-room restart and
the three-role procedure in `CLEAN_ROOM.md` completes. The source-reading
specification role, isolated implementation role, and independent expression
audit role must have separate access boundaries; the trace must bind the
maintainer decision, behavioral specification, restarted attempt, audit, and
mechanical wall-integrity result. Compatible licensing and all required notices
...
2. The required Linux x86-64 and current Apple Silicon (macOS 26) CPU jobs pass.
...
That structural validator is not the Kaname verifier. It cannot establish that
the private history exists, that capture was complete, that the full ancestor
graph closes, or that the evidence accepts the exact candidate. During the
temporary closure, the trusted-base workflow therefore also rejects pull
...
It must read the private ledger, verify the canonical evidence graph and custody
receipts, replay every binding for the exact candidate and every newly reachable
commit and merge parent, reject stale or reconstructed evidence, and publish an
app-owned required status without exposing ledger credentials to pull-request
workflows. External contributions remain closed until that gate and its negative
controls have been validated and the repository policy is explicitly reopened.

## Bootstrap record

The initial gate-installation and internal-only policy commits are bootstrap
policy events because the production verifier was not yet active. They did not
...
all later changes must satisfy the post-enforcement contract.
